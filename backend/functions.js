...
const generateFiles = async (
  ...
  const execPromise = util.promisify(exec); // Use util.promisify.custom for Alpine Linux
...
const cloneAndGenerateDiff = async (
  ...
  try {
    ...
    const { stdout: cloneOutput } = await execPromise(
      `git clone ${req.body.linkRepo}`,
      {
        cwd: path.join(__dirname, `./clones/${id}`),
        shell: "/bin/sh", // Use '/bin/sh' for Alpine Linux
      }
    );
    await execPromise(`cd ./clones/${id}/${reponame.repositoryName}`);
    let { stdout: diffOutput } = await execPromise(command, {
...
const diff = async (req, local, id) => {
  const command = "git diff HEAD HEAD~1";
  ...
  if (local) {
    ...
    await fs.promises.mkdir(path.join(__dirname, `./clones/${key}`));
